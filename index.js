...
const scriptVariables = {
  customThemeFilePath: path.join(
    process.cwd(),
    process.argv[2] || "./custom-theme.less"
  ),
  generatedThemeFilePath: path.join(
    process.cwd(),
    process.argv[3] || "./custom-theme.css"
  ),
  antdLibraryPath: path.join(
    process.cwd(),
    process.argv[4] || "./node_modules/antd"
  ),
};
...
exec(
  ...
  (error1, stdout, stderr) => {
    // console.debug(stdout);s
    if (!error1) {
      console.debug(
        `Dynamic generation: "/tmp/generated-theme.less" tmp file was successfully generated`
      );
      exec(
        `lessc --js ${path.join(__dirname, "/tmp/generated-theme.less")} ${
          scriptVariables.generatedThemeFilePath
        }`,
        { cwd: __dirname },
